...
export const ChatView: React.FC<ChatViewProps> = ({ isDark = false }) => {
  ...
  return (
    <div className={`flex flex-col h-full border backdrop-blur-sm max-w-3xl mx-auto rounded-sm overflow-hidden transition-all duration-1000 ${isDark ? 'bg-[#111111] border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
      ...
      {mode === 'chat' && (
        <>
          ...
          <div className={`p-4 border-t ${isDark ? 'border-zinc-800 bg-[#111111]' : 'border-zinc-200 bg-white'}`}>
