  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): { gl: WebGLRenderingContext | WebGL2RenderingContext; ext: any } | null {
      ...
      if (isWebGL2) {
        ...
        supportLinearFiltering = gl.getExtension("OES_texture_float_linear") as OES_texture_float_linear;
      } else {
        halfFloat = gl.getExtension("OES_texture_half_float") as OES_texture_half_float;
        supportLinearFiltering = gl.getExtension("OES_texture_half_float_linear") as OES_texture_half_float_linear;
      }
    ...
    function initFramebuffers() {
      let simRes = getResolution(config.SIM_RESOLUTION, canvas);
      let dyeRes = getResolution(config.DYE_RESOLUTION, canvas);
    ...
    function splatPointer(pointer: any, canvas: HTMLCanvasElement) {
      let dx = pointer.deltaX * config.SPLAT_FORCE;
      let dy = pointer.deltaY * config.SPLAT_FORCE;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color, canvas);
    }

    function clickSplat(pointer: any, canvas: HTMLCanvasElement) {
      ...
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, color, canvas);
    }

    function splat(x: number, y: number, dx: number, dy: number, color: any, canvas: HTMLCanvasElement) {
      splatProgram.bind();
    ...
    function updatePointerDownData(pointer: any, id: number, posX: number, posY: number, canvas: HTMLCanvasElement) {
      pointer.id = id;
      pointer.down = true;
      pointer.moved = false;
    ...
    function updatePointerMoveData(pointer: any, posX: number, posY: number, color: any, canvas: HTMLCanvasElement) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
    ...
    function getResolution(resolution: number, canvas: HTMLCanvasElement): { width: number; height: number } {
