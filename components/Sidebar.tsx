...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, onBook }) => {
  ...
  return (
    ...
    <div className="w-full md:w-64 bg-zinc-50 dark:bg-[#050505] border-r border-zinc-200 dark:border-r-0 flex flex-col h-auto md:h-full sticky top-0 z-50 transition-colors duration-[1000ms]">
            ...
            >
              {item.id === View.HOME ? (
                <img 
                  ...
                  alt="Home" 
                  className={`w-5 h-5 object-contain dark:invert transition-all duration-[1000ms] ${isActive ? 'opacity-100' : 'opacity-60 group-hover:opacity-100'}`}
                />
              ) : (
      ...
      <div className="hidden md:block p-4 space-y-4">
        <button 
          onClick={onBook}
          className="flex items-center justify-center gap-2 w-full border border-zinc-200 dark:border-none dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:border-black hover:text-black hover:bg-white dark:hover:text-white dark:hover:bg-zinc-800 px-4 py-4 transition-all duration-[1000ms] text-[10px] uppercase tracking-[0.2em] font-bold"
        >
          Book Now
        </button>
