...
/**
 * Generates chat response using Gemini Flash
 * Persona: Studio Assistant for 'emotional studios' (Self-Portrait Studio)
 */
export const generateChatResponse = async (history: {role: string, parts: {text: string}[]}[], newMessage: string): Promise<string> => {
  try {
    ...
    return result.text || "I apologise, I'm strictly focused on our studio enquiries at the moment.";
  } catch (error) {
